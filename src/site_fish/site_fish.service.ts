...
import {
  SiteFish,
  SiteFishServiceInterface,
} from './interfaces/site_fish.interface';
...
export class SiteFishService implements SiteFishServiceInterface {
  ...
  async addFishToSite(dto: AddFishToSiteDto): Promise<SiteFish> {
    try {
      const created = await this.siteFishRepository.addFishToSite(
        dto.site_id,
        dto.fish_id,
      );
      ...
      if (error.code === '23505') {
        ...
        throw new ConflictException(
          'This fish is already associated with this site',
        );
      }
      if (error.code === '23503') {
  ...
  async addBulkFishToSite(dto: AddBulkFishToSiteDto): Promise<SiteFish[]> {
    try {
      const created = await this.siteFishRepository.addBulkFishToSite(
        dto.site_id,
        dto.fish_ids,
      );
      ...
      if (error.code === '23505') {
        throw new ConflictException(
          'One or more fish are already associated with this site',
        );
      }
      if (error.code === '23503') {
        throw new NotFoundException(
          'One or more fish or the site were not found',
        );
      }
      throw error;
    }
  ...
  async addBulkSitesToFish(dto: AddBulkSitesToFishDto): Promise<SiteFish[]> {
    try {
      const created = await this.siteFishRepository.addBulkSitesToFish(
        dto.fish_id,
        dto.site_ids,
      );
      return created.map(this.toSiteFishInterface);
      ...
      if (error.code === '23505') {
        throw new ConflictException(
          'This fish is already associated with one or more sites',
        );
      }
      if (error.code === '23503') {
