...
import {
  FishingSite,
  FishingSitesServiceInterface,
} from './interfaces/fishing_site.interface';
import { FishingSitesRepository } from 'src/databases/repositories/fishing_sites/fishing_sites.repository';
import {
  CreateFishingSiteDto,
  FindAllDto,
  UpdateFishingSiteDto,
} from './dtos/fishing_site.dto';
import { max } from 'class-validator';

@Injectable()
export class FishingSitesService implements FishingSitesServiceInterface {
  constructor(
    private readonly fishingSitesRepository: FishingSitesRepository,
  ) {}
  async create(
    createFishingSiteDto: CreateFishingSiteDto,
  ): Promise<FishingSite> {
    const existing = await this.fishingSitesRepository.findByName(
      createFishingSiteDto.name,
    );
    existing.forEach((site) => {
      if (site.name.toLowerCase() === createFishingSiteDto.name.toLowerCase()) {
        throw new ConflictException(
          'Fishing site with this name already exists',
        );
      }
    });

    const created =
      await this.fishingSitesRepository.create(createFishingSiteDto);
  ...
  async update(
    id: number,
    updateFishingSiteDto: UpdateFishingSiteDto,
  ): Promise<FishingSite> {
    const site = await this.fishingSitesRepository.findById(id);
    if (!site) throw new NotFoundException('Fishing site not found');

    const updated = await this.fishingSitesRepository.update(
      id,
      updateFishingSiteDto,
    );
