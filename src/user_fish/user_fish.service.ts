...
import {
  UserFishServiceInterface,
  UserFish,
} from './interfaces/user_fish.interface';
...
export class UserFishService implements UserFishServiceInterface {
  ...
  async updateUserFish(
    user_id: number,
    fish_id: number,
    dto: UpdateUserFishDto,
  ): Promise<UserFish> {
    const existing = await this.repository.findByUserIdAndFishId(
      user_id,
      fish_id,
    );
