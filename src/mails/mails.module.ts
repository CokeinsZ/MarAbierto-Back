  ...
  imports: [
    ...
    MailerModule.forRootAsync({
      ...
      useFactory: async (config: ConfigService) => {
        ...
        const rejectUnauthorized = (config.get<string>('MAIL_TLS_REJECT_UNAUTHORIZED') || 'true').toLowerCase() !== 'false';

        logger.log(`Mail transport => host=${host} port=${port} secure=${secure} rejectUnauthorized=${rejectUnauthorized}`);
