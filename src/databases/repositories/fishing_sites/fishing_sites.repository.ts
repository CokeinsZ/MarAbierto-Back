...
export class FishingSitesRepository {
  ...
  async findById(id: number): Promise<FishingSite | null> {
    const rows = await this.db
      .query<FishingSite>`SELECT * FROM fishingsites WHERE site_id = ${id}`;
    return rows[0] || null;
  }

  async findByName(name: string): Promise<FishingSite[]> {
    return this.db
      .query<FishingSite>`SELECT * FROM fishingsites WHERE unaccent(name) ILIKE '%' || unaccent(${name}) || '%'`;
  }

  async filterByAddress(address: string): Promise<FishingSite[]> {
    return this.db
      .query<FishingSite>`SELECT * FROM fishingsites WHERE unaccent(address) ILIKE '%' || unaccent(${address}) || '%'`;
  }
