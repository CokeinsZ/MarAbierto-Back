...
export class UsersService implements UserServiceInterface {
  ...
  async changePassword(id: number, dto: ChangePasswordDto): Promise<void> {
    ...
    if (!code) throw new NotFoundException('Verification code not found, please request a new one');
    if (code.code !== dto.code) throw new UnauthorizedException('Invalid verification code');
    if (new Date() > code.expires_at) throw new UnauthorizedException('Verification code expired');

    const user = await this.userRepository.findById(id);
    if (!user) throw new NotFoundException('User not found');
    if (user.status !== user_status.ACTIVE) throw new UnauthorizedException('User is not active');
  ...
  async updateStatus(id: number, statusDto: UpdateUserStatusDto): Promise<User> {
    const user = await this.userRepository.updateUserStatus(id, statusDto.status);
    if (!user) throw new NotFoundException('User not found');
    return this.toUserInterface(user);
  }
  ...
  async login(loginDto: LoginDto): Promise<{ message: string; userId: string; email: string; role: string; token: string }> {
    const user = await this.userRepository.findByEmail(loginDto.email);
    if (!user) throw new NotFoundException('User not found');
    if (user.status !== user_status.ACTIVE) {
      if (user.status === user_status.NOT_VERIFIED) throw new UnauthorizedException('Please verify your email');
      if (user.status === user_status.BANNED) throw new UnauthorizedException('User is banned');
    ...
    return { message: 'Login successful', userId: user.user_id.toString(), email: user.email, role: user.role, token };
  }
